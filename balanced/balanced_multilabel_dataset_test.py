...
def create_pie(classes, method, label_tag, class_tag, processed_data_folder):
    ...
    fig.savefig(
        processed_data_folder + r"/pies/multilabel/test/pie_" + f"{str(method)}_" + pie_label_tag + "_" + class_tag + ".png",
        bbox_inches='tight')
...
class PtbXlClassesSuperclassesMultilabelBalanced(PtbXlClassesSuperclasses):
    def __init__(
        ...
        self.y_val_multilabel = self.prepare_labels_encoding_val(tabular, self.balanced_multilabel_encoder)
        self.y_test_multilabel = self.prepare_labels_encoding_test(tabular, self.balanced_multilabel_encoder)
    ...
    def prepare_labels_encoding_val(self, tabular: pd.DataFrame, le) -> np.ndarray:
        labels_encoding = le.transform(tabular["multilabel_superclass"].to_numpy())
        labels_encoding_val = labels_encoding[self.labels_indices_val]
        return labels_encoding_val

    def prepare_labels_encoding_test(self, tabular: pd.DataFrame, le) -> np.ndarray:
        labels_encoding = le.transform(tabular["multilabel_superclass"].to_numpy())
        labels_encoding_val = labels_encoding[self.labels_indices_test]
        return labels_encoding_val
    ...
    def balanced_by_imbalanced_learn_method(self, method):
        ...
        if not (
                os.path.exists(os.path.join(
                    self.processed_data_folder + r"/x_resampled_multilabel_test", f"multilabel_test_{str(method)}.npy")
                )
                and os.path.exists(os.path.join(
            self.processed_data_folder + r"/y_resampled_multilabel_test", f"multilabel_test_{str(method)}.npy")
        )
        ):
            # X = self._waves_train
            ...
            # X = np.concatenate((self._waves_val, self._waves_test), axis=0)
            # y = np.concatenate((self.y_val_multilabel, self.y_test_multilabel), axis=0)
            ...
            create_pie(y_resampled_decoded_counter, str(method), "Multi-label_test", "subclasses",
            ...
            np.save(os.path.join(self.processed_data_folder + r"/x_resampled_multilabel_test",
                                 f"multilabel_test_{str(method)}"),
                    X_resampled)
            np.save(os.path.join(self.processed_data_folder + r"/y_resampled_multilabel_test",
                                 f"multilabel_test_{str(method)}"),
                    y_resampled)

        with open(os.path.join(
                self.processed_data_folder + r"/x_resampled_multilabel_test",
                f"multilabel_test_{str(method)}.npy"
        ), "rb") as x_file:
            X_resampled = np.load(x_file)

        with open(os.path.join(
                self.processed_data_folder + r"/y_resampled_multilabel_test",
                f"multilabel_test_{str(method)}.npy"
        ), "rb") as y_file:
...
def main():
    ...
    # # RandomUnderSampler
    # X_resampled_rus, y_resampled_rus = dataset.balanced_by_imbalanced_learn_method(RandomUnderSampler(sampling_strategy='not minority'))
    ...
    # X_resampled_nm3_neighbor, y_resampled_nm3_neighbor = dataset.balanced_by_imbalanced_learn_method(NearMiss(version=3, n_neighbors_ver3=5))
    ...
    # balanced_classification(dataset, X_resampled_nm3_neighbor, y_resampled_nm3_neighbor, 'NearMiss-3__multilabel_n_neighbors_ver3=14', 30)
    ...
    #                         'EditedNearestNeighbours_multilabel_n_neighbors=2_kind_sel=mode', 10)
    ...
    #                         'EditedNearestNeighbours_multilabel_n_neighbors=3_kind_sel=all', 10)
    ...
    # balanced_classification(dataset, X_resampled_renn, y_resampled_renn, 'RepeatedEditedNearestNeighbours_kind_sel=all_max_iter=100', 10)

    # AllKNN (over-sampling cleaning)
    # X_resampled_allknn, y_resampled_allknn = dataset.balanced_by_imbalanced_learn_method(
    #     AllKNN())
    # balanced_classification(dataset, X_resampled_allknn, y_resampled_allknn, 'AllKNN_sampling_kind_sel=all_n_neighbors=3', 10)

    # NeighbourhoodCleaningRule (over-sampling)
    # X_resampled_ncr, y_resampled_ncr = dataset.balanced_by_imbalanced_learn_method(NeighbourhoodCleaningRule(kind_sel='mode'))
    # balanced_classification(dataset, X_resampled_ncr, y_resampled_ncr, 'NeighbourhoodCleaningRule_multilabel_kind_sel=mode', 10)
    ...
    # balanced_classification(dataset, X_resampled_cnn, y_resampled_cnn, 'CondensedNearestNeighbour', 10)
    #
    # X_resampled_cnn, y_resampled_cnn = dataset.balanced_by_imbalanced_learn_method(
        # CondensedNearestNeighbour(n_neighbors=1))
    # balanced_classification(dataset, X_resampled_cnn, y_resampled_cnn, 'CondensedNearestNeighbour_1_neighbors', 10)
    #
    # X_resampled_cnn, y_resampled_cnn = dataset.balanced_by_imbalanced_learn_method(
    #     CondensedNearestNeighbour(n_neighbors=5))
    # balanced_classification(dataset, X_resampled_cnn, y_resampled_cnn, 'CondensedNearestNeighbour_5_neighbors', 10)
